...
let variable = scene.createRenderable(zLayer, (image: Image, camera: scene.Camera) => {
    for (let x1 = 0; x1 < 10; x1++) {
        for (let y1 = 0; y1 < 18; y1++) {
            redrawImg.drawImg(x1 * 20 + y1 % 2 * 10 - 10 - camera.x % 20, Math.round(y1 / 2) * 16 - y1 % 2 * 8 - 8 - camera.y % 16, list[tileMapImg.getPixel(x1 + Math.round(mySprite.x / 16), Math.round(y1 + mySprite.y / 16))], image)
        }
    }
